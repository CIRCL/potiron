...
def output_name(outputdir, source, date, field_in_file_name, fieldvalues):
    ...
    if lentwo and all_proto:
        return "{}{}_{}_{}_with-protocols{}".format(outputdir, source, date, field_in_file_name, value_str)
    else:
        return "{}{}_{}_{}{}".format(outputdir, source, date, field_in_file_name, value_str)
...
#    links = args.links
    ...
    namefile = output_name(outputdir, source, date, field_in_file_name, fieldvalues)
    ...
    for v in range(vlength): # For each selected field or occurrence
        ...
        if len(value) >= 2:
            ...
            if ck:
                for m in range(1,13):
                    days = calendar.monthrange(int(date),m)[1]
                    month = format(m, '02d')
                    for d in range(1,days+1):
                        exists_day = False
                        day = format(d, '02d')
                        countValue = 0
                        keys = red.keys("{}:*:{}{}{}:{}".format(source,date,month,day,field))
                        for k in keys:
                            redisKey = k.decode()
    ...
    if at_least_one:
        ...
        p.title.text = "Number of {} {}seen each day in {}".format(field_string, fieldvalues_string, date)
